...
if (process.argv.length < 5) {
  console.log("Usage: <structurizrUrl> <png|svg> dir [username] [password]")
  process.exit(1);
}
...
(async () => {
  ...
  for (var i = 0; i < views.length; i++) {
    ...
    if (format === SVG_FORMAT) {
      const diagramFilename = path.resolve(dir, IMAGE_DIR, FILENAME_SUFFIX + view.key + '.svg');
      const diagramKeyFilename = path.resolve(dir, IMAGE_DIR, FILENAME_SUFFIX + view.key + '-key.svg');
      ...
      const diagramFilename =path.resolve(dir, IMAGE_DIR, FILENAME_SUFFIX + view.key + '.png');
      const diagramKeyFilename = path.resolve(dir, IMAGE_DIR, FILENAME_SUFFIX + view.key + '-key.png');
